...
namespace saba
{
	...
	*/
	...
	struct PMXPackedVertexData
	{
		struct AddUV
		{
			glm::vec4 m_data[4];
			void set(glm::vec4 data[4])
			{
				m_data[0] = data[0];
				m_data[1] = data[1];
				m_data[2] = data[2];
		...
		std::vector<AddUV> m_addUV;
		std::vector<int> m_weightType;
	...
	struct PMXFile
	{
		...
		PMXPackedVertexData m_packed_vertex_data;
	};
