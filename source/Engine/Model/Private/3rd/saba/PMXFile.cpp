...
namespace saba
{
	namespace
	{
		...
		void PackedVertexData(std::vector<PMXVertex>& vertices, PMXPackedVertexData& packed_data)
		{
			int size = vertices.size();
			packed_data.resize(size);
			for (int i = 0; i < size; i++)
			{
				packed_data.m_position[i] = vertices[i].m_position;
				packed_data.m_normal[i] = vertices[i].m_normal;
				packed_data.m_uv[i] = vertices[i].m_uv;
				packed_data.m_addUV[i].set(vertices[i].m_addUV);
				packed_data.m_weightType[i] = (int)vertices[i].m_weightType;
				packed_data.m_boneIndices[i].set(vertices[i].m_boneIndices);
				packed_data.m_boneWeights[i].set(vertices[i].m_boneWeights);
				packed_data.m_sdefC[i] = vertices[i].m_sdefC;
				packed_data.m_sdefR0[i] = vertices[i].m_sdefR0;
				packed_data.m_sdefR1[i] = vertices[i].m_sdefR1;
				packed_data.m_edgeMag[i] = vertices[i].m_edgeMag;
				if (packed_data.m_weightType[i] == 1)
				{
					packed_data.m_boneWeights[i].m_data[1] = 1 - packed_data.m_boneWeights[i].m_data[0];
				}
			}
		}
	}
	...
	bool ReadPMXFile(PMXFile * pmxFile, const char* filename)
	{
		...
		PackedVertexData(pmxFile->m_vertices, pmxFile->m_packed_vertex_data);
