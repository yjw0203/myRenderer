...
namespace rhi
{
    uint32_t findMemoryType__(VkPhysicalDevice gpu, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    void vkCreateBuffer(VkDevice device,VkPhysicalDevice gpu, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
    {
        ...
        allocInfo.memoryTypeIndex = findMemoryType__(gpu, memRequirements.memoryTypeBits, properties);
